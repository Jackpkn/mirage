...
```typescript
import { HF, HfModelsResource, RAMResource, Workspace } from '@struktoai/mirage-node'
...
The Hub resources need no extra: they speak the Hub API over `fetch`. `@struktoai/mirage-node` is a different product, the
Xet object store behind [HF Buckets](/typescript/setup/hf_buckets).
