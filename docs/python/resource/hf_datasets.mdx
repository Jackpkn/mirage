...
Every read command in [HF Buckets'](/python/resource/hf_buckets#read-commands) set works here, as
do the text processing and path utilities, which only read. What does not is
the [File Operations](/python/resource/hf_buckets#file-operations) group: this mount is read-only,
