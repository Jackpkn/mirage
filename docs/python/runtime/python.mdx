...
| `sandlock` | The host `python` interpreter, confined by [sandlock](https://github.com/multikernel/sandlock) (Landlock + seccomp). Linux only | Only the host paths the config grants | No |
...
## <img noZoom src="/images/sandlock-logo.svg" alt="" className="inline-block h-5 align-middle mr-2" />Sandlock

`sandlock` is `local` with a fence around it: the same host CPython, the
same native wheels and exact `sys.flags`, but spawned through
[sandlock](https://github.com/multikernel/sandlock) so a Landlock ruleset
and a seccomp filter bound where its I/O can land. Nothing is granted
except the interpreter's own tree, the system paths CPython needs to
start, and what you list:
...
It needs the sandlock CLI on `PATH` — there is no extra to install,
because the CLI is the transport. **Linux only**, and the full ruleset
wants Landlock ABI v6 (Linux 6.12+); sandlock's own degrade options cover
older kernels with weaker protection.

Two things to know before reaching for it:

- **It does not see workspace mounts either.** Its reach is `process`,
  the same as `local`: confinement narrows *which* host paths the code
  can touch, but the workspace gate never sees that I/O, so mount modes,
  policy, and accounting do not apply. What it buys is that a `process`
  runtime need no longer be an unbounded one. Point `fs_writable` at a
  [FUSE-mounted](/python/setup/fuse) workspace path and the code works on
  your mounts while reaching nothing else on the host.
- **The environment is not inherited.** `local` passes `os.environ`
  straight through; `sandlock` starts empty and sets only the config
  `env` plus the run's own, so confined code is never handed the backend
  credentials mirage is holding. Anything the code needs must be named:
