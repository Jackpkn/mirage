...
The `monty`, `wasi`, `sandlock`, and `local` runtimes run `python3` on your
own machine. A **sandbox runtime** does the opposite: it takes a whole
command line and runs it inside a separate machine, a local container or
...
The smolvm CLI is the transport, so there is no SDK dependency and no
extra: the machine gets real stdin and separated stderr, exactly as the
container does. Host directories reach the guest only if you passed them
at boot (`--volume`), because the guest runs its own kernel and sees
nothing of your filesystem otherwise.
