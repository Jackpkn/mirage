...
<Note>
  ...
  because it isolates execution, but it is not the same runtime tier. It
  confines only the host `python3` interpreter; the four providers above run
  arbitrary captured command lines in another container, VM, or machine.
</Note>
...
sandboxes: **you bring your own** (a running container or microVM, or a live
Daytona or E2B sandbox), the first captured line connects to it, and every
captured line execs inside it verbatim.
...
- **Docker and smolvm** have no server-side timeout: a runaway runs until you
  stop it or the container or microVM exits.
