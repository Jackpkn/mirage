...
const alloc = (p) => {
  ...
  const id = nextId++;
  byPath.set(p, id); byId.set(id, p);
  return id;
};
const join2 = (d, n) => (d === "/" ? "/" + n : d + "/" + n);
const attrsOf = (id, p) => {
  if (links.has(p)) return { fileid: id, size: Buffer.byteLength(links.get(p)), isDir: false, isSymlink: true };
...
const delegate = {
  ...
  setSize: async ({ id, size }) => {
    const p = byId.get(id);
    if (size !== undefined && size !== null && files.has(p)) files.set(p, files.get(p).subarray(0, size));
    return attrsOf(id, p);
  },
  ...
  write: async ({ id, offset, data }) => {
    ...
    base.copy(merged); data.copy(merged, offset);
    files.set(p, merged);
    return attrsOf(id, p);
  },
  ...
  rename: async ({ fromDirId, fromName, toDirId, toName }) => {
    ...
    if (files.has(src)) { files.set(dst, files.get(src)); files.delete(src); }
    if (dirs.has(src)) { dirs.add(dst); dirs.delete(src); }
    const id = byPath.get(src);
    if (id !== undefined) { byPath.delete(src); byPath.set(dst, id); byId.set(id, dst); }
    return {};
  },
  ...
  readdir: async ({ dirId, startAfter, maxEntries }) => {
    ...
    for (const n of sorted) {
      ...
      if (resuming) { if (id === startAfter) resuming = false; continue; }
...
const handle = await start(
  delegate.lookup, delegate.getattr, delegate.setSize, delegate.read,
  delegate.write, delegate.create, delegate.mkdir, delegate.remove,
  delegate.rename, delegate.symlink, delegate.readlink, delegate.readdir,
  delegate.flushIdle, "127.0.0.1", 0, 1, process.getuid(), process.getgid(), 5.0);
...
const checks = [];
const sh = async (...argv) => { try { const { stdout } = await run(argv[0], argv.slice(1)); return [0, stdout.trim()]; } catch (e) { return [e.code ?? 1, (e.stdout ?? "").trim() + (e.stderr ?? "").trim()]; } };
let [c, o] = await sh("ls", mnt);          checks.push([`ls -> ${o}`, o.includes("hello.txt") && o.includes("docs")]);
[c, o] = await sh("cat", `${mnt}/hello.txt`);   checks.push([`cat -> ${o}`, o === "hello-from-node"]);
[c, o] = await sh("cat", `${mnt}/docs/n.txt`);  checks.push([`nested -> ${o}`, o === "nested"]);
[c, o] = await sh("sh", "-c", `echo via-ts > ${mnt}/new.txt`); checks.push(["write", c === 0]);
[c, o] = await sh("cat", `${mnt}/new.txt`);     checks.push([`read-back -> ${o}`, o === "via-ts"]);
[c, o] = await sh("sh", "-c", `mkdir ${mnt}/d && mv ${mnt}/new.txt ${mnt}/d/m.txt`); checks.push(["mkdir+mv", c === 0]);
[c, o] = await sh("cat", `${mnt}/d/m.txt`);     checks.push([`moved -> ${o}`, o === "via-ts"]);
[c, o] = await sh("ln", "-s", "hello.txt", `${mnt}/lnk`); checks.push(["ln -s", c === 0]);
[c, o] = await sh("readlink", `${mnt}/lnk`);    checks.push([`readlink -> ${o}`, o === "hello.txt"]);
[c, o] = await sh("cat", `${mnt}/lnk`);         checks.push([`through link -> ${o}`, o === "hello-from-node"]);
[c, o] = await sh("rm", `${mnt}/lnk`);          checks.push(["rm link", c === 0]);
[c, o] = await sh("cat", `${mnt}/hello.txt`);   checks.push(["target survives", o === "hello-from-node"]);
