...
class CommandRule:
    ...
    about) matching commands on matching paths.

    It is the element type of ``commands.deny`` and ``commands.ask`` at
    every tier and reaches the workspace only inside that document; the
    internal RulePolicy is what evaluates it. A command entry is a
    token-prefix pattern over the line as the door normalizes it
    (``rm`` is every rm line, ``git push`` every ``git push ...``, a
    ``*`` token any one token). Path entries use the document's one
    grammar: an entry with ``*``, ``?`` or ``[`` is a pattern (repo
    fnmatch dialect, ``*`` crossing ``/``, a slashless pattern matching
    any name component), anything else is an exact path and its
    subtree.

    Args:
        reason (str): why the command is refused, shown on stderr.
        commands (tuple[str, ...]): command patterns the rule applies
            to; empty means every command.
