...
async def _listing_error(accessor: HfBucketsAccessor, path: PathSpec,
                         target: str) -> OSError:
    """The errno for a path the tree API listed nothing at all under.

    Args:
        accessor (HfBucketsAccessor): Hugging Face bucket accessor.
...
async def readdir(accessor: HfBucketsAccessor,
    ...
    if not saw_entry and target.strip("/"):
        ...
        # exactly while it holds a key and any entry at all proves it.
        # The Hub answers a missing subpath with 200 and [], which the
        # lister reports as an empty result rather than raising, so the
        # NotFound arm above never fired and `ls /hf/never` rendered an
        # empty directory and exited 0. The mount root is exempt: it
        # exists because it is mounted.
