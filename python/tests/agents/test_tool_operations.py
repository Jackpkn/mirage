...
async def test_write_then_read_back(ops, workspace):
    ...
    assert result.is_error is False
    assert await workspace.ops.read("/new.txt") == b"hello world"
...
async def test_write_refuses_existing(ops, workspace):
    ...
    assert await workspace.ops.read("/exists.txt") == b"first"
...
async def test_write_creates_parents(ops, workspace):
    ...
    assert result.is_error is False
    assert await workspace.ops.read("/nested/deep/file.txt") == b"hi"
...
async def test_edit_replaces_once(ops, workspace):
    ...
    assert result.is_error is False
    assert await workspace.ops.read("/edit.txt") == b"foo qux baz"
...
async def test_edit_refuses_ambiguous(ops, workspace):
    ...
    assert await workspace.ops.read("/multi.txt") == b"aa bb aa"
...
async def test_edit_replace_all(ops, workspace):
    ...
    assert await workspace.ops.read("/all.txt") == b"cc bb cc"
